    ...
    return (
        <div className="flex flex-col gap-2 max-w-[280px]">
            <div className="font-semibold text-sm flex items-center gap-2 text-secondary">
                ...
                {tt("Durable Session (Attached)")}
            </div>
            <div className="text-xs text-secondary leading-relaxed">
                {tt("Your shell state, running programs, and history are protected. This session will survive network disconnects.")}
            </div>
            <LearnMoreButton />
        </div>
    );
    ...
    return (
        <div className="flex flex-col gap-2 max-w-[280px]">
            <div className="font-semibold text-sm flex items-center gap-2 text-secondary">
                ...
                {tt("Durable Session (Detached)")}
            </div>
            <div className="text-xs text-secondary leading-relaxed">
                {tt("Connection lost, but your session is still running on the remote server. Wave will automatically reconnect when the connection is restored.")}
            </div>
            <LearnMoreButton />
        </div>
    );
...
function DurableAwaitingStart({ connected, viewModel, onClose }: DurableAwaitingStartProps) {
        ...
        return (
            <div className="flex flex-col gap-2 max-w-[280px]">
                <div className="font-semibold text-sm flex items-center gap-2 text-secondary whitespace-nowrap">
                    ...
                    {tt("Durable Session (Awaiting Connection)")}
                </div>
                <div className="text-xs text-secondary leading-relaxed">
                    {tt("Configured for a durable session. The session will start when the connection is established.")}
                </div>
                <LearnMoreButton />
            </div>
        );
    ...
    return (
        <div className="flex flex-col gap-2 max-w-[280px]">
            <div className="font-semibold text-sm flex items-center gap-2 text-secondary whitespace-nowrap">
                <i className="fa-sharp fa-solid fa-shield text-muted" />
                {tt("Durable Session (Awaiting Start)")}
            </div>
            <div className="text-xs text-secondary leading-relaxed">
                {tt("Configured for a durable session, but session hasn't started yet. Click below to start it manually.")}
            </div>
            <button
                className="bg-zinc-700 text-foreground rounded px-3 py-1.5 text-xs font-medium hover:bg-zinc-600 transition-colors cursor-pointer flex items-center justify-center gap-2 mt-1"
...
function DurableStartingContent({ onClose }: DurableStartingContentProps) {
    ...
    return (
        <div className="flex flex-col gap-2 max-w-[280px]">
            <div className="font-semibold text-sm flex items-center gap-2 text-secondary">
                <i className="fa-sharp fa-solid fa-shield text-sky-300" />
                {tt("Durable Session (Starting)")}
            </div>
            <div className="text-xs text-secondary leading-relaxed">{tt("The durable session is starting.")}</div>
            <LearnMoreButton />
        </div>
    );
}
...
function DurableEndedContent({ doneReason, startupError, viewModel, onClose }: DurableEndedContentProps) {
    ...
    let titleText = tt("Durable Session (Ended)");
    let descriptionText = tt("The durable session has ended. This block is still configured for durable sessions.");
    const showRestartButton = true;

    if (doneReason === "terminated") {
        titleText = tt("Durable Session (Ended, Exited)");
        descriptionText =
            tt("The shell was terminated and is no longer running. This block is still configured for durable sessions.");
    } else if (doneReason === "gone") {
        titleText = tt("Durable Session (Ended, Lost)");
        descriptionText =
            tt("The session was lost or not found on the remote server. This may have occurred due to a system reboot or the session being manually terminated.");
    } else if (doneReason === "startuperror") {
        titleText = tt("Durable Session (Failed to Start)");
        descriptionText = tt("The durable session failed to start.");
        return (
            <div className="flex flex-col gap-2 max-w-[280px]">
                <div className="font-semibold text-sm flex items-center gap-2 text-secondary">
                    <i className="fa-sharp fa-solid fa-shield text-muted" />
